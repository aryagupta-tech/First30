...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (duplicate && operation.existing && duplicate.status === 'failed') {
      ...
      await env.FILES.put(duplicate.object_key, bytes, { httpMetadata: { contentType: file.type }, customMetadata: { synthetic: 'true', sha256: digest } });
      await env.DB.batch([
    ...
    try { await env.FILES.put(objectKey, bytes, { httpMetadata: { contentType: file.type }, customMetadata: { synthetic: 'true', sha256: digest } }); }
