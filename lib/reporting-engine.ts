...
export async function processSubmissionWorkflow(input: { sessionId: string; caseId: string; requestId: string; gateway?: SubmissionGateway }) {
  ...
  try {
    ...
    await addEvent(input.caseId, 'submitted', 'Mock complaint received', 'मॉक शिकायत प्राप्त', `${submission?.acknowledgement || result.acknowledgement} was created inside FIRST30. No external system was contacted.`, `${submission?.acknowledgement || result.acknowledgement} FIRST30 के भीतर बनाया गया। किसी बाहरी सिस्टम से संपर्क नहीं हुआ।`, completed);
    await addEvent(input.caseId, 'action_required', 'Additional evidence requested · Mock', 'अतिरिक्त प्रमाण अनुरोध · मॉक', 'The mock review asks for a synthetic bank statement.', 'मॉक समीक्षा काल्पनिक बैंक स्टेटमेंट माँगती है।', completed + 1);
